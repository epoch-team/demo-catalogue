...
RUN export GIT_SSL_NO_VERIFY=1 && gvt restore && \
    CGO_ENABLED=0 GOOS=linux go build -a -installsuffix cgo -o /app ./cmd/cataloguesvc
...
RUN	chmod +x /app && \
	...
	apk add sudo && sudo setcap 'cap_net_bind_service=+ep' /app
